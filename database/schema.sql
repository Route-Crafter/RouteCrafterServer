...
DROP TABLE IF EXISTS route_way_executions_union_segments;
DROP TABLE IF EXISTS routes_ways;
DROP TABLE IF EXISTS way_coords;
...
-- Tabas referentes a la uniión de las rutas
CREATE TABLE ways (
	id INT PRIMARY KEY,
    name VARCHAR(150) NOT NULL,
    highway VARCHAR(120) NOT NULL,
    max_speed_kmh FLOAT
);

CREATE TABLE way_coords (
	id INT AUTO_INCREMENT PRIMARY KEY,
    way_id INT REFERENCES ways(id)
		ON DELETE CASCADE,
	lat FLOAT NOT NULL,
    lon FLOAT NOT NULL,
    INDEX idx_wc_way (way_id)
);

CREATE TABLE routes_ways(
	id INT AUTO_INCREMENT PRIMARY KEY,
    route_id BINARY(16) REFERENCES routes(id)
		ON DELETE CASCADE,
    way_id INT REFERENCES ways(id)
		ON DELETE CASCADE,
    ordered_position INT NOT NULL,
    CONSTRAINT unique_route_way UNIQUE (route_id, way_id),
	CONSTRAINT unique_route_order UNIQUE (route_id, ordered_position),
    INDEX idx_rw_route (route_id),
	INDEX idx_rw_way (way_id)
);

CREATE TABLE route_way_executions_union_segments(
	id INT AUTO_INCREMENT PRIMARY KEY,
    route_id BINARY(16) REFERENCES routes(id)
		ON DELETE CASCADE,
	way_id INT REFERENCES ways(id)
	...
	p1 FLOAT NOT NULL,
    p2 FLOAT NOT NULL,
    INDEX idx_rweus_route (route_id),
	INDEX idx_rweus_way (way_id)
);
